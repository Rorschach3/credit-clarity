...
export const useFileUploadHandler = ({
  ...
  // Storage-based upload that triggers Edge Function processing
  const handleStorageUpload = useCallback(async (event: ChangeEvent<HTMLInputElement>) => {
    console.log("🚀 ===== Starting storage-based upload process =====");
    ...
    try {
      ...
      if (!user?.id) {
        throw new Error('User authentication required for storage upload');
      }
      ...
      // Stage 3: Trigger Edge Function processing
      setUploadProgress(PROGRESS_STAGES.PROCESSING);
      console.log("🔄 Triggering Edge Function processing...");

      // Manual trigger as fallback (if storage triggers aren't configured)
      const { data: edgeFunctionData, error: edgeFunctionError } = await supabase.functions.invoke('process-credit-report', {
        body: {
          record: {
      ...
      if (edgeFunctionError) {
        console.warn("⚠️ Edge Function manual trigger error:", edgeFunctionError);
        // Don't throw - storage triggers might still work
      } else {
        console.log("✅ Edge Function triggered successfully:", edgeFunctionData);
      }

      // Stage 4: Poll for results
      setUploadProgress(PROGRESS_STAGES.PARSING);
      console.log("🔍 Waiting for processing results...");

      const tradelines = await pollForTradelineResults(user.id, Date.now());
      ...
      if (tradelines.length > 0) {
        ...
        const extractedText = `Processed ${tradelines.length} tradelines from ${file.name}`;
        ...
        sonnerToast.success(`Successfully processed ${tradelines.length} tradelines!`);
      } else {
        throw new Error('No tradelines were extracted from the document');
      }
  ...
  // Helper function to poll for tradeline results
  const pollForTradelineResults = useCallback(async (userId: string, startTime: number, maxWaitTime = 120000): Promise<ParsedTradeline[]> => {
    const pollInterval = 3000; // 3 seconds
    
    while (Date.now() - startTime < maxWaitTime) {
      try {
        console.log("🔍 Polling for tradeline results...");
        
        // Query for recent tradelines
        const { data: tradelines, error } = await supabase
          .from('tradelines')
          .select('*')
          .eq('user_id', userId)
          .gte('created_at', new Date(startTime - 10000).toISOString()) // 10 seconds buffer
          .order('created_at', { ascending: false });

        if (error) {
          console.error("❌ Error querying tradelines:", error);
        } else if (tradelines && tradelines.length > 0) {
          console.log(`✅ Found ${tradelines.length} new tradelines`);
          
          // Convert to ParsedTradeline format
          const parsedTradelines: ParsedTradeline[] = tradelines.map(t => ({
            id: t.id,
            user_id: t.user_id,
    ...
    throw new Error('Processing timeout - please check your file and try again');
  }, []);
