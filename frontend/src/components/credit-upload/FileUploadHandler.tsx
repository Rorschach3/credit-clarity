...
const processCreditReportWithAPI = async (file: File, userId: string): Promise<ParsedTradeline[]> => {
  ...
  // For local endpoints with FormData, don't set any headers to avoid interfering with multipart/form-data
  const finalHeaders = endpoint.includes('localhost') ? {} : headers;
  console.log(`📋 Final headers being sent:`, Object.keys(finalHeaders));
